 ...
 * @param methodIdentifiers Object containing all methods identifier
 */
function getOverriddenSender(userdoc, signature, methodIdentifiers) {
...
function runTest(testName, testObject, contractDetails, fileAST, opts, testCallback, resultsCallback) {
    ...
    async_1.default.eachOfLimit(runList, 1, function (func, index, next) {
        ...
        if (func.constant) {
            ...
            if (web3.remix && web3.remix.registerCallId)
                web3.remix.registerCallId(tagTimestamp);
            method.call(sendParams).then((result) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                const time = (Date.now() - startTime) / 1000.0;
                let tagTxHash;
                if (web3.remix && web3.remix.getHashFromTagBySimulator)
                    tagTxHash = yield web3.remix.getHashFromTagBySimulator(tagTimestamp);
                if (web3.remix && web3.remix.getHHLogsForTx)
                    hhLogs = yield web3.remix.getHHLogsForTx(tagTxHash);
            ...
            method.send(sendParams).on('receipt', (receipt) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                try {
                    debugTxHash = receipt.transactionHash;
                    if (web3.remix && web3.remix.getHHLogsForTx)
                        hhLogs = yield web3.remix.getHHLogsForTx(receipt.transactionHash);
                    const time = (Date.now() - startTime) / 1000.0;
                    ...
                    for (const i in receipt.logs) {
                        ...
                        for (const event of events) {
                            ...
                            if (eIndex >= 0) {
                                const testEvent = web3.eth.abi.decodeParameters(assertionEvents_1.default[eIndex].params, event.data);
                                if (!testEvent[0]) {
                ...
                const time = (Date.now() - startTime) / 1000.0;
                if (failedTransactions[err.receipt.transactionHash])
                    return; // we are already aware of this transaction failing.
                failedTransactions[err.receipt.transactionHash] = time;
                let errMsg = err.message;
                let txHash;
                if (err.reason)
                    errMsg = `transaction reverted with the reason: ${err.reason}`;
                const resp = {
                    type: 'testFailure',
                    value: changeCase.sentenceCase(func.name),
                    filename: testObject.filename,
                ...
                if (err.receipt)
                    txHash = err.receipt.transactionHash;
                ...
                if (web3.remix && web3.remix.getHHLogsForTx && txHash)
                    hhLogs = yield web3.remix.getHHLogsForTx(txHash);
                if (hhLogs && hhLogs.length)
                    resp.hhLogs = hhLogs;
