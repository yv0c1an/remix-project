...
var _remixSimulator = __webpack_require__(28482);
var provider = null;
self.onmessage = function (e) {
  ...
  switch (data.cmd) {
    case 'init':
      {
        provider = new _remixSimulator.Provider({
          fork: data.fork,
          nodeUrl: data.nodeUrl,
          blockNumber: data.blockNumber,
          stateDb: data.stateDb,
          blocks: data.blocks
        });
    ...
    case 'sendAsync':
      {
        if (provider) {
          provider.sendAsync(data.query, function (error, result) {
            self.postMessage({
              cmd: 'sendAsyncResult',
              error: error,
              result: result,
    ...
    case 'newAccount':
      {
        if (provider) {
          provider.Accounts.newAccount(function (error, address) {
            if (error) {
              self.postMessage({
...
/******/ })()
;
//# sourceMappingURL=src_blockchain_providers_worker-vm_ts-libs_remix-lib_src_util_ts.0.52.0-dev.1720620484381.js.map